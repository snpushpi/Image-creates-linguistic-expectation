...
```{r}
closed_data <- dataset_with_error %>% filter(POS=="Closed")
...
#saveRDS(model_closed_gpt2, 'saved_models/model_closed_gpt2')
model_closed_gpt2 <- readRDS('saved_models/model_closed_gpt2')

summary(model_closed_gpt2)
...
#saveRDS(model_closed_kosmos2, 'saved_models/model_closed_kosmos2')
model_closed_kosmos2 <- readRDS('saved_models/model_closed_kosmos2')

summary(model_closed_kosmos2)
```
```{r}
...
```{r}
model_A <- model_closed_gpt2
model_B <- model_closed_kosmos2
...
filtered_data$term <- factor(filtered_data$term, levels = c("Condition_ID.helm1","Condition_ID.helm2","surprisal"), labels = c("RT facilitation for Wrong vs no","RT facilitation for Correct vs wrong&no mean","RT facilitation for 1 unit increase in surprisal"))
...
ggplot(filtered_data, aes(x = term, y = estimate, fill = model)) +
  ...
  scale_x_discrete(labels = function(x) 
    stringr::str_wrap(x, width = 10))+
    labs(x = "Effect", y = "RT Facilitation Estimate and 95% CI, closed class words") +
  theme_minimal()
```
